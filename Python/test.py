...
# Custom CSS for theming and button styling
st.markdown("""
    <style>
    body {
...
def show_question(q, section_name, question_index):
    st.subheader(f"Question {question_index + 1} — {section_name}")
    st.write(q['question'])

    options = q['options']
    key_prefix = f"{section_name}_{question_index}"

    selected = st.radio("Choose an option:", options, key=f"radio_{key_prefix}")

    # Columns for side-by-side buttons
    col1, col2 = st.columns([1, 1])

    # State keys
    submit_key = f"submitted_{key_prefix}"
    show_key = f"show_answer_{key_prefix}"
    ...
    with col1:
        if st.button("✅ Submit", key=f"submit_btn_{key_prefix}"):
            st.session_state[submit_key] = True
            st.session_state[show_key] = False

    with col2:
        if st.button("🔍 Show Answer", key=f"show_btn_{key_prefix}"):
    ...
    if st.session_state[submit_key]:
        ...
        st.info(f"ℹ️ Correct answer: **{correct_option}**\n\n{q['explanation']}")
